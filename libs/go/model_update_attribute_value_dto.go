/*
 * SQE API
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * API version: v1
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// UpdateAttributeValueDTO struct for UpdateAttributeValueDTO
type UpdateAttributeValueDTO struct {
	Id int32 `json:"id"`
	Value string `json:"value"`
	Description NullableString `json:"description,omitempty"`
	CssDirectives NullableString `json:"cssDirectives,omitempty"`
}

// NewUpdateAttributeValueDTO instantiates a new UpdateAttributeValueDTO object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewUpdateAttributeValueDTO(id int32, value string, ) *UpdateAttributeValueDTO {
	this := UpdateAttributeValueDTO{}
	this.Id = id
	this.Value = value
	return &this
}

// NewUpdateAttributeValueDTOWithDefaults instantiates a new UpdateAttributeValueDTO object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewUpdateAttributeValueDTOWithDefaults() *UpdateAttributeValueDTO {
	this := UpdateAttributeValueDTO{}
	return &this
}

// GetId returns the Id field value
func (o *UpdateAttributeValueDTO) GetId() int32 {
	if o == nil  {
		var ret int32
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *UpdateAttributeValueDTO) GetIdOk() (*int32, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *UpdateAttributeValueDTO) SetId(v int32) {
	o.Id = v
}

// GetValue returns the Value field value
func (o *UpdateAttributeValueDTO) GetValue() string {
	if o == nil  {
		var ret string
		return ret
	}

	return o.Value
}

// GetValueOk returns a tuple with the Value field value
// and a boolean to check if the value has been set.
func (o *UpdateAttributeValueDTO) GetValueOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Value, true
}

// SetValue sets field value
func (o *UpdateAttributeValueDTO) SetValue(v string) {
	o.Value = v
}

// GetDescription returns the Description field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *UpdateAttributeValueDTO) GetDescription() string {
	if o == nil || o.Description.Get() == nil {
		var ret string
		return ret
	}
	return *o.Description.Get()
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *UpdateAttributeValueDTO) GetDescriptionOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return o.Description.Get(), o.Description.IsSet()
}

// HasDescription returns a boolean if a field has been set.
func (o *UpdateAttributeValueDTO) HasDescription() bool {
	if o != nil && o.Description.IsSet() {
		return true
	}

	return false
}

// SetDescription gets a reference to the given NullableString and assigns it to the Description field.
func (o *UpdateAttributeValueDTO) SetDescription(v string) {
	o.Description.Set(&v)
}
// SetDescriptionNil sets the value for Description to be an explicit nil
func (o *UpdateAttributeValueDTO) SetDescriptionNil() {
	o.Description.Set(nil)
}

// UnsetDescription ensures that no value is present for Description, not even an explicit nil
func (o *UpdateAttributeValueDTO) UnsetDescription() {
	o.Description.Unset()
}

// GetCssDirectives returns the CssDirectives field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *UpdateAttributeValueDTO) GetCssDirectives() string {
	if o == nil || o.CssDirectives.Get() == nil {
		var ret string
		return ret
	}
	return *o.CssDirectives.Get()
}

// GetCssDirectivesOk returns a tuple with the CssDirectives field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *UpdateAttributeValueDTO) GetCssDirectivesOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return o.CssDirectives.Get(), o.CssDirectives.IsSet()
}

// HasCssDirectives returns a boolean if a field has been set.
func (o *UpdateAttributeValueDTO) HasCssDirectives() bool {
	if o != nil && o.CssDirectives.IsSet() {
		return true
	}

	return false
}

// SetCssDirectives gets a reference to the given NullableString and assigns it to the CssDirectives field.
func (o *UpdateAttributeValueDTO) SetCssDirectives(v string) {
	o.CssDirectives.Set(&v)
}
// SetCssDirectivesNil sets the value for CssDirectives to be an explicit nil
func (o *UpdateAttributeValueDTO) SetCssDirectivesNil() {
	o.CssDirectives.Set(nil)
}

// UnsetCssDirectives ensures that no value is present for CssDirectives, not even an explicit nil
func (o *UpdateAttributeValueDTO) UnsetCssDirectives() {
	o.CssDirectives.Unset()
}

func (o UpdateAttributeValueDTO) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["id"] = o.Id
	}
	if true {
		toSerialize["value"] = o.Value
	}
	if o.Description.IsSet() {
		toSerialize["description"] = o.Description.Get()
	}
	if o.CssDirectives.IsSet() {
		toSerialize["cssDirectives"] = o.CssDirectives.Get()
	}
	return json.Marshal(toSerialize)
}

type NullableUpdateAttributeValueDTO struct {
	value *UpdateAttributeValueDTO
	isSet bool
}

func (v NullableUpdateAttributeValueDTO) Get() *UpdateAttributeValueDTO {
	return v.value
}

func (v *NullableUpdateAttributeValueDTO) Set(val *UpdateAttributeValueDTO) {
	v.value = val
	v.isSet = true
}

func (v NullableUpdateAttributeValueDTO) IsSet() bool {
	return v.isSet
}

func (v *NullableUpdateAttributeValueDTO) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableUpdateAttributeValueDTO(val *UpdateAttributeValueDTO) *NullableUpdateAttributeValueDTO {
	return &NullableUpdateAttributeValueDTO{value: val, isSet: true}
}

func (v NullableUpdateAttributeValueDTO) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableUpdateAttributeValueDTO) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


